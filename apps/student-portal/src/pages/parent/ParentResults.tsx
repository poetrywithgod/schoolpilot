...
export const ParentResults = () => {
    ...
    const load = async () => {
      ...
      try {
        ...
        Object.values(groups).forEach((group) => {
          ...
          group.average = group.results.length > 0 ? Math.round(total / group.results.length) : 0
        })

        const sortedGroups = Object.values(groups).sort((_, b) => (b.isCurrent ? 1 : -1))
  ...
  return (
    <PageLayout title={child ? `${child.first_name}'s Results` : 'Results'} showBack hideNav>
        ...
        {isLoading ? (
          ...
          <div className="text-center py-16">
            ...
            <p className="text-sm text-gray-400" style={{ fontFamily: 'Lora, serif' }}>
              Results will appear here once published by teachers
            </p>
          </div>
        ) : (
          <div className="space-y-3">
            {grouped.map((group) => (
              <div key={group.termName} className="bg-white rounded-2xl overflow-hidden shadow-sm">
                <button
                  onClick={() => setExpandedTerm(expandedTerm === group.termName ? null : group.termName)}
                ...
                >
                  <div className="flex items-center gap-3">
                    ...
                    >
                      <Trophy size={18} style={{ color: group.isCurrent ? '#FFBA00' : '#9ca3af' }} />
                    </div>
                  ...
                  <ChevronRight
                    ...
                    style={{ transform: expandedTerm === group.termName ? 'rotate(90deg)' : 'rotate(0deg)' }}
                  />
                </button>
                ...
                {expandedTerm === group.termName && (
                  <div className="border-t border-gray-100">
                    ...
                    {group.results.map((result) => {
                      ...
                      return (
                        <div key={result.id} className="grid grid-cols-12 gap-2 px-4 py-3 border-t border-gray-50 items-center">
                          <div className="col-span-5">
                            <p className="text-xs font-medium text-gray-900 truncate" style={{ fontFamily: 'Poppins, sans-serif' }}>
                              {subjectName}
                            </p>
                            <p className="text-xs font-bold mt-0.5" style={{ color: '#0C3B2E', fontFamily: 'Poppins, sans-serif' }}>
                          ...
                          <div className="col-span-2 flex justify-center">
                            <span
                              ...
                              style={{ backgroundColor: gc.bg, color: gc.color, fontFamily: 'Poppins, sans-serif' }}
                            >
                              {result.grade}
                            </span>
                          </div>
                    ...
                    >
                      <p className="text-xs text-gray-500" style={{ fontFamily: 'Lora, serif' }}>
                        Term Average
                      </p>
