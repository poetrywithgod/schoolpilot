...
export const ParentAttendance = () => {
  ...
  useEffect(() => {
    const load = async () => {
      ...
      try {
        ...
        const term = await getCurrentTermForSchool(childData.school_id)
        setTermName(term.name)
        const data = await getStudentAttendance(childId, term.id)
        setSummary(data)
      } catch (err: any) {
  ...
  return (
    <PageLayout title={child ? `${child.first_name}'s Attendance` : 'Attendance'} showBack hideNav>
            ...
            {!summary || summary.records.length === 0 ? (
              <div className="text-center py-16">
                ...
                <p className="text-sm text-gray-400" style={{ fontFamily: 'Lora, serif' }}>
                  Records will appear here once the school marks attendance
                </p>
              </div>
            ) : (
